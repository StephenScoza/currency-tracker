import type { FxPoint, SignalAssessment, TrendDirection } from "../types/currency";
...
const movingAverage = (points: FxPoint[], windowSize: number) => {
  ...
  return average(slice.map((point) => point.rate));
};

const percentileRank = (points: FxPoint[], currentRate: number) => {
  const belowOrEqual = points.filter((point) => point.rate <= currentRate).length;
  return belowOrEqual / Math.max(points.length, 1);
};
...
export const buildSignalAssessment = (points: FxPoint[]): SignalAssessment => {
  ...
  let recommendation: SignalAssessment["recommendation"] = "NEUTRAL";
  ...
  const confidenceBase = Math.abs(percentile - 0.5) * 120 + Math.abs(momentum) * 800 + Math.abs(rangePosition - 0.5) * 35;
